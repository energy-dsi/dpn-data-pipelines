...
# +---------+----------------------------------------------------------+---------------+-------------+
"""
Adaptor — DL file ingestion.

Polls the dl-stage container for new files, copies each to
...
class AdaptorFileProcess:
    """
    File-ingestion adaptor for DL.
    Reads from dl-stage, copies to mapper staging, publishes Kafka events.
    """
...
def run(ctx: PipelineContext) -> None:
    ...
    step_log.pipeline_banner(
        ctx,
        service_name="producer-file-dl-adaptor",
        config_summary={
