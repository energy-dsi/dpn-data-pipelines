...
# +---------+----------------------------------------------------------+---------------+-------------+
"""
Airflow DAG — Producer · File · BP Natural Gas (Kafka trigger pattern)
...
orchestrates the existing pods by publishing trigger messages to Kafka.
  ...
  Task 1: trigger_adaptor
    ...
    The bp-natural-gas adaptor pod receives it, runs one cycle (list files
...
BOOTSTRAP_SERVER = os.getenv("bootstrapServer", "dpn-kafka-src:9092")
SCHEDULE         = '*/3 * * * *'
...
with DAG(
    dag_id          = "producer_file_bp_natural_gas",
    default_args    = _DEFAULT_ARGS,
    description     = "BP Natural Gas file pipeline — Kafka trigger, no new pods",
    schedule_interval = SCHEDULE,
    start_date      = days_ago(1),
    catchup         = False,
    max_active_runs = 1,
    tags            = ["dpn", "producer", "file", "bp-natural-gas", "kafka-trigger"],
) as dag:
    ...
    trigger_adaptor = PythonOperator(
        ...
        doc_md = (
            "Publishes a trigger message to dpn-pipeline-control for the bp-natural-gas adaptor. "
