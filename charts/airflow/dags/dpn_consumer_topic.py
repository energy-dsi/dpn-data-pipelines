...
"""
Airflow DAG: consumer_topic

Orchestrates the already-running Kubernetes pods for consumer-topic via Kafka.
...
Task chain:
  trigger_extractor → wait_extractor_done → trigger_schema_mapper → wait_schema_mapper_done
"""
...
PRODUCT  = "consumer-topic"
P_TYPE   = "topic"
P_ROLE   = "consumer"
STAGE1   = "extractor"
# BOOTSTRAP = os.getenv("bootstrapServer","")
BOOTSTRAP = "dpn-kafka-target:9092"
# SCHEDULE  = os.getenv("CONSUMER_TOPIC_SCHEDULE","@hourly")
# SCHEDULE  = "*/3 * * * *"
SCHEDULE  = None
...
with DAG(dag_id="consumer_topic",default_args=_DEFAULT,
         description="consumer-topic — Kafka trigger, existing pods, no new K8s pods",
         schedule_interval=SCHEDULE,start_date=days_ago(1),
         catchup=False,max_active_runs=1,
         tags=["dpn","consumer","topic","consumer-topic","kafka-trigger"]) as dag:
