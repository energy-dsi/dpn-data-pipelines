...
class TopicSchemaMapper:
    ...
    def _resolve_target(self, headers: dict) -> None:
        ...
        schema_type = _sanitize(
            headers.get("schema_type") or headers.get("schematype") or headers.get("schemaType")
        )
        org_name = _sanitize(
            headers.get("org_name") or headers.get("orgname") or  headers.get("orgName")
        )
        product_type = _sanitize(
            headers.get("product_type") or headers.get("producttype") or  headers.get("productType")
        )
        ...
        try:
            with self.tracer.start_as_current_span("process_message") as span:
                ...
                try:
                    ...
                    enriched_headers = [
                        ("schemaType", _sanitize(headers.get("schema_type") or headers.get("schematype") or headers.get("schemaType")).encode()),
                        ("orgName", _sanitize(headers.get("org_name") or headers.get("orgname") or headers.get("orgName")).encode()),
                        ("productType", _sanitize(headers.get("product_type") or headers.get("producttype")or headers.get("productType")).encode()),
